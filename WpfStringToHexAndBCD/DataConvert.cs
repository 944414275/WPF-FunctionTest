...
namespace TransmissionSW.Common.Model
{
    public class DataConvert
    {
        ...
        /// <summary>
        /// 智能转换：HEX码字符串到byte数组，遇到0~F、空格以外的字符马上退出
        /// </summary>
        ...
        public byte[] HexString2HexByte(string strInput)
        {
            ...
            foreach (char c in strInput)//20191118 Kmola 遍历字符串中的每一个字符
            {
                //首先验证范围
                if ((c >= (char)'0' && c <= (char)'9') || (c >= (char)'A' && c <= (char)'F'))//20191118 Kmola 查看字符串中的每一个字符是否是0-9或a-f，即该字符串是否是16进制的格式
                {
